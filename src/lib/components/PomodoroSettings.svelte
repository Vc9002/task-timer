<script lang="ts">
  ...
  async function save() {
    try {
      const settings = { work_minutes: workMinutes, break_minutes: breakMinutes };
...
<section>
  <h2>Pomodoro</h2>
  <p class="muted">Set your work and break lengths. Start it anytime from the sidebar, for whatever task you're on.</p>
...
<style>
  .pomodoro-grid { display: flex; gap: 20px; margin: 12px 0; }
