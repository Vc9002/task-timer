<script lang="ts">
  ...
  let error = $state("");
  let groups = $derived(Array.from(new Set(tasks.map(task => `${task.course_code}\u0000${task.class_id}`))).map(key => {
    const [course_code, classId] = key.split("\u0000");
    return { course_code, class_id: Number(classId), tasks: tasks.filter(task => task.class_id === Number(classId)) };
  }));
