<script lang="ts">
  import { getCalendar, scheduleTask, type CalendarDay } from "$lib/api";
  ...
  async function move(taskId: number, date: string) {
    moving = taskId; error = "";
    try { await scheduleTask(taskId, date || null); await refresh(); }
    ...
    <div class="grid">
      ...
      {#each days as day (day.date)}
        <section class="day" class:today={day.date === localDate()}>
          ...
          {#if day.study_blocks.length}<p class="kind block-label">STUDY BLOCKS</p>{#each day.study_blocks as block (block.id)}<div class="task block"><span>{block.task_title}<small>{block.planned_start_time ? `${block.planned_start_time} · ` : ""}{block.planned_minutes}m</small></span></div>{/each}{/if}
          {#if day.planned.length}<p class="kind planned-label">PLANNED TASKS</p>{#each day.planned as task (task.id)}<div class="task planned"><span>{task.title}<small>{task.estimated_minutes ? `${task.estimated_minutes}m` : ""}</small></span><input aria-label={`Move ${task.title}`} type="date" value={day.date} disabled={moving === task.id} onchange={(e) => void move(task.id, e.currentTarget.value)} /></div>{/each}{/if}
          {#if day.due.length}<p class="kind due-label">DUE</p>{#each day.due as task (task.id)}<div class="task due"><span>{task.title}</span><input aria-label={`Schedule ${task.title}`} type="date" value={task.scheduled_date ?? ""} disabled={moving === task.id} onchange={(e) => void move(task.id, e.currentTarget.value)} /></div>{/each}{/if}
        </section>
      {/each}
    </div>
