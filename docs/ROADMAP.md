...
4. **Analytics quality.** Estimate calibration, weekly review, semester trends, planned-vs-actual, deadline coverage, class workload.
   - Date-normalization inconsistency (2026-09-17): `today.rs`/`week.rs`/`planner.rs` all compare due dates via a documented `CASE WHEN length(due_at)=10 THEN date(due_at) ELSE date(due_at,'localtime') END` pattern (date-only strings are floating local dates; timestamp deadlines convert via local time), but `analytics.rs` had three places — `workload_days`, `schedule_coverage`, and `get_weekly_review`'s overdue count — using bare `date(due_at)`, which misdates a timestamp-style due value by treating it as UTC. This directly affects deadline coverage and workload numbers for any task with a timestamp due date. Fixed by extracting the shared pattern into a `due_date_expr()` helper and applying it at all three sites. `cargo test --lib` (79 passing, no regressions), `cargo clippy -D warnings`, `cargo fmt --check`, `npm run check` all clean. No new test added — the day-boundary effect is timezone-dependent and a deterministic unit test for it would be flaky across machines; same gap pre-existed for the equivalent logic in today.rs/week.rs/planner.rs.
   - Estimate calibration (median-based, 3-sample minimum) and semester dashboard were reviewed and look correct — no changes needed there.
   - Remaining: not yet reviewed — planned-vs-actual reporting depth, and whether class workload should also account for `blocked_by_open_count` the way the planner now does (a blocked task's "remaining due minutes" may currently overstate near-term workload if it can't actually be worked on yet).
5. **Operational reliability.** Backup/restore verification, migration testing, export validation, crash recovery, sleep/wake behavior.
   - Backup/restore fix (2026-09-17): same bug class as the planner dependency gap — `task_dependencies` (added by a later feature commit) was never added to `restore_backup`'s orphan-pruning list, alongside study_blocks/task_milestones/exams/task_templates/recurring_task_templates/session_notifications. A restore that removed a task could leave stale `task_dependencies` rows referencing a now-nonexistent task id, which could silently reattach to an unrelated task if SQLite ever reused that rowid, corrupting the "blocked by" feature. Fixed: `task_dependencies` now pruned on either side (`task_id` or `depends_on_task_id`) not present in the restored tasks. Extended the existing restore test to cover it. `cargo test --lib` (79 passing), `cargo clippy -D warnings`, `cargo fmt --check`, `npm run check` all clean.
   - Migration runner reviewed: sequential, and there's already a test covering a failed migration rolling back both schema and the recorded version — looks solid, no changes made.
   - Sleep/wake reviewed: tracked time is derived from `start_ts`/`end_ts` timestamps, not an in-memory running counter, so the app going to sleep mid-session can't desync tracked duration — already correctly designed and covered by an existing test (`sleep_gap_and_paused_gap_are_timestamp_derived`).
   - Remaining: crash recovery (there's a `recovery`/`recoverTimer` path per the frontend timer store — not yet audited), and export validation beyond the tests that already exist for CSV/ICS/JSON backup format.
